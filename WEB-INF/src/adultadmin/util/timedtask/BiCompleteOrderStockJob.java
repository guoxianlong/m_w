 ...
 */
@Component
public class BiCompleteOrderStockJob implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		System.out.println(DateUtil.getNow() + " 定时任务统计复核作业环节作业量 开始执行");
		...
		param.setOperType(EBIOperType.Type5.getIndex());
		...
		synchronized (lock) {		
			try {
				...
				sb.append(" SELECT areano AS areaId, COUNT(order_id) AS operCount ");
				sb.append(" FROM audit_package  ");
				sb.append(" WHERE check_datetime BETWEEN '").append(yesterday).append(" 00:00:00' AND '").append(yesterday).append(" 23:59:59' ");
				sb.append(" AND status >= 2 ");
				sb.append(" GROUP BY areano  ");
				...
				System.out.println(DateUtil.getNow() + " 定时任务统计复核作业环节作业量 执行结束");		
			} catch (Exception e) {
				e.printStackTrace();
				db.rollbackTransaction();
				System.out.println(DateUtil.getNow() + " 定时任务统计复核作业环节作业量 发生异常");
			}
			finally{
				db.release();
				dbSlave.release();
