...
public class JdOrderFulfillmentJob implements Job {
	public static byte[] lock = new byte[0];
    //京东订单配送确认上传数定时任务
	String appKey = "418773602E3761FE84E443D24C9402A6";
	 String appSecret = "05cf9fd00721453eae0106ede68d4f64";
	 String accessToken = "b9ace4a2-8fcc-4c99-8f3d-81557f234562";
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		synchronized (lock) {
			...
			try {
				System.out.println("京东订单配送确认修改订单状态任务开始");
				...
				String query = "SELECT uop.pop_order_id,jom.order_id,jom.order_stock_id,ap.package_code,ap.order_code, dci.phone FROM jd_order_message jom  "
				...
				if( joiList.size() != 0 ) {
					for( JdOrderInfoBean joiBean : joiList ) {
						if( "0".equals((joiBean.getLogisticsId())) ) {
							...
							String updateSql = "UPDATE jd_order_message SET "
				...
				System.out.println("京东订单配送确认修改订单状态任务结束");
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				service.releaseAll();
