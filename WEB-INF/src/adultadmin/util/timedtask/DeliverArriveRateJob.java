 ...
 * @date 2014-4-3
 */
public class DeliverArriveRateJob implements Job{
	public static byte[] deliverArriveRateLock = new byte[0];
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("到达当地及时率统计定时任务开始");
		synchronized (deliverArriveRateLock) {
			...
			try{
				...
				if(deliverList!=null && deliverList.size()>0){
					...
					for(int i=0;i<deliverList.size();i++){
						...
						//发货仓id为3,4
						List<DeliverArriveRate> list = new ArrayList<DeliverArriveRate>();
						DeliverArriveRate bean = getDeliverArriveRate(dbOp, deliver,3);
						if(bean!=null){
							list.add(bean);
						}
						bean = getDeliverArriveRate(dbOp, deliver,4);
						if(bean!=null){
							list.add(bean);
						}
						if(list!=null && list.size()>0){
							StringBuilder insertSql = new StringBuilder();
							insertSql.append(" insert into deliver_arrive_rate ");
							insertSql.append(" (deliver_id,area_id,date,transit_count,intime_arrive_count,intime_arrive_rate) ");
							insertSql.append(" values (?,?,?,?,?,?) ");
							pst = dbOp.getConn().prepareStatement(insertSql.toString());
							for(int j=0;j<list.size();j++){
								DeliverArriveRate arriveRate = list.get(j); 
								pst.setInt(1, arriveRate.getDeliverId());
								pst.setInt(2, arriveRate.getAreaId());
								pst.setDate(3, new java.sql.Date(arriveRate.getDate().getTime()));
								pst.setInt(4, arriveRate.getTransitCount());
								pst.setInt(5, arriveRate.getIntimeArriveCount());
								if(arriveRate.getTransitCount()!=0){
									pst.setFloat(6, Arith.div(arriveRate.getIntimeArriveCount(), arriveRate.getTransitCount(), 4));
								}else{
									pst.setFloat(6, 0);
								}
								pst.addBatch();
		...
		System.out.println("到达当地及时率统计定时任务结束");
	}

	private DeliverArriveRate getDeliverArriveRate(DbOperation dbOp, DeliverCorpInfoBean deliver,int area) throws SQLException {
		//查询到达当地时效
		String arriveTime = "";
		ResultSet rs = dbOp.executeQuery("select arrive_time from deliver_kpi where deliver_id=" + deliver.getId() + " and area_id=" + area);
		if(rs.next()){
		...
		if(arriveTime.length()>0){
			...
			.append(" and doi.deliver_state=4 and FROM_UNIXTIME(doi.add_time/1000,'%Y-%m-%d %H:%i:%s') < DATE_ADD(mb.transit_datetime,INTERVAL ")
			.append(Integer.valueOf(arriveTime).intValue()).append(" HOUR) where ap.areano=").append(area).append(" and ap.deliver=")
			.append(deliver.getId());
			rs = dbOp.executeQuery(collectCountSql.toString());
			if(rs.next()){
				bean.setIntimeArriveCount(rs.getInt("amount"));
			}
			return bean;
		}else{
			return null;
