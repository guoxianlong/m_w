...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		tree = $('#tree').tree({
			url : '${pageContext.request.contextPath}/HelpContext/getViewTrees.mmx',
			animate : false,
			onClick : function(node) {
				var obj = tree.tree('getSelected');
				console.info(node.attributes.from);
				if (node.attributes && node.attributes.url && node.attributes.url != '') {
					...
					if (/^\//.test(node.attributes.src)) {/*以"/"符号开头的,说明是本项目地址*/
						...
						href = '${pageContext.request.contextPath}/admin/help/helpcontext.jsp?menuId='+ obj.id ;
					}
					addTabFun({
						src : href,
						title : node.text
...
onclick="collapseAll();">折叠</a>
					<a href="javascript:void(0);" class="easyui-linkbutton" plain="true" iconCls="icon-

reload" onclick="tree.tree('reload');">刷新</a>
