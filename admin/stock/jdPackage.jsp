...
<table id="__01"  border="1" cellpadding="0" cellspacing="0" style="font-family: SimHei;">
	...
	<tr>
		<td colspan="5" style="width: 100mm;height: 18mm;" align="left" >
			<img src="<%=request.getContextPath()%>/image/logo_fhd.bmp" width="90px" height="50px"/></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan="3" >
			地址:<font style="font-size: 14px;font-weight: bold;">买卖宝<%=ProductStockBean.areaMap.get(osBean.getStockArea()) %>仓</font>
			...
			发货人:无锡买卖宝&nbsp;&nbsp;&nbsp;&nbsp;<%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %>
			</td>
	 ...
	 <tr>
		...
		保留此包裹单底联，代表您已签收并认可我们配送的商品及发货明细单背面规定的内容
		 <%if (!order.isTaobaoOrder()) { %>
				，如有任何疑问，可通过以下方式联系mmb客服中心<strong>【电话：40088-43211】</strong></td>
		<%} %>
	</tr>
	<tr>
		<td colspan="5" style="width: 100mm;height: 17mm;" align="left" >
			<%if (order.getDeliver() != 45 &&order.getDeliver() != 50 &&order.getDeliver() != 26 &&order.getDeliver() != 27 &&order.getDeliver()!=55) {%>
			<img src="<%=request.getContextPath()%>/image/logo_fhd.bmp" width="90px" height="50px"/>
			<%} %>
		</td>
	</tr>
...
function initPrint(){
	...
	if(LODOP.PRINT_INIT("")){ //mmb发货清单打印
		...
		LODOP.ADD_PRINT_BARCODE("3mm", "30mm","70mm", "15mm", "128A", "<%=orderCode.toUpperCase()%>");
		LODOP.ADD_PRINT_BARCODE("91mm", "9mm","12mm", "10mm", "128A", "<%=order.getCode()%>");
		<%if (order.getDeliver() != 45 &&order.getDeliver() != 50 &&order.getDeliver() != 26 &&order.getDeliver() != 27 && order.getDeliver() !=55) {%>
			LODOP.ADD_PRINT_BARCODE("132mm", "30mm","70mm", "15mm", "128A", "<%=orderCode.toUpperCase()%>");
		<%} else {%>  
