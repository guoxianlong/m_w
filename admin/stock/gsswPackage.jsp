...
<table id="__01"  border="1" cellpadding="0" cellspacing="0" style="font-family: SimHei; table-layout:fixed;word-break:break-all;">
	...
	<tr>
		...
		<%if(order.getDeliver()==37){ %>
			地址：<font style="font-weight: bold;">买卖宝<%=ProductStockBean.areaMap.get(osBean.getStockArea()) %>仓（无锡邮政）</font><br>
			发货人:买卖宝&nbsp;&nbsp;&nbsp;&nbsp;<%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %>
		<%}else if(order.getDeliver()==29){ %>
			地址：<font style="font-weight: bold;">买卖宝<%=ProductStockBean.areaMap.get(osBean.getStockArea()) %>仓（泰州邮政）</font><br>
			发货人:买卖宝&nbsp;&nbsp;&nbsp;&nbsp;<%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %>
		...
		<%}else if(order.getDeliver()==52){ %>
			寄件人：无锡买卖宝	<font style="margin-left:30px"><%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %></font><br>
			寄件地址：买卖宝成都仓
		<%}else{ %>
			寄件人：赵艳	<font style="margin-left:30px"><%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %></font><br>
			寄件地址：广州市站南路4号
		<%} %>
		</td>
	</tr>
	...
	<tr>
		<td colspan="5" style="width: 105mm; height: 9mm;font-size:13px;overflow:hidden;">
			...
			<%}else if(order.getDeliver()==52){ %>
				...
				寄件地址：买卖宝成都仓
			<%}else{ %>
				寄件人：赵艳&nbsp;<%if (!order.isTaobaoOrder()) { %>电话：40088-43211<%} %><br>
				寄件地址：广州市站南路4号
			<%} %>
		</td>
	</tr>
