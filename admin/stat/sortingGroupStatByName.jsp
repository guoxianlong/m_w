...
function checkDate(){
    ...
    if (!re.test(dateStart)&&dateStart!=""&&dateEnd!=""){
         alert('分播日期的起始日期，请输入正确的格式！如：2011-10-09');
         return false;
    }
    if (!re.test(dateEnd)&&dateStart!=""&&dateEnd!=""){
        alert('分播日期的截止日期，请输入正确的格式！如：2011-10-09');
        return false;
   }
	return true;
}
...
<body>
&nbsp;&nbsp;&nbsp;个人分播量统计
<form action="sortingGroupStatAction.do?method=sortingGroupStatByName" method="post" onsubmit="return checkDate();">
分播人：<input type="text" name="userName" maxlength="45" value="<%=userName %>"/>
