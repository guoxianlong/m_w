...
<body>
&nbsp;&nbsp;&nbsp;每日分播量统计
<form action="sortingGroupStatAction.do?method=sortingGroupStatByDate" method="post">
分播月份：
	...
	<%for(int j=0;j<statArray[0].length;j++){ %>
		<%if(i==0){ %>
			<%
				if( j == 0 ) {
			%>
				<td><%if(j==0||j==(statArray[0].length-1)){ %><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font><%}else{ %><a href="sortingGroupStatAction.do?method=sortingGroupStatByName&userName=<%=statArray[i][j]==null?"":statArray[i][j] %>&dateStart=<%=year %>-<%=month<10?"0"+month:month %>-01&dateEnd=<%=year %>-<%=month<10?"0"+month:month %>-<%=dayCount<10?"0"+dayCount:dayCount %>" target="_blank" ><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font></a><%} %></td>
			<%	
				}
			%>
			<%
				if( (j-1)%3 == 0 ) {
			%>
				<td colspan="3"><%if(j==0||j==(statArray[0].length-1)){ %><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font><%}else{ %><a href="sortingGroupStatAction.do?method=sortingGroupStatByName&userName=<%=statArray[i][j]==null?"":statArray[i][j] %>&dateStart=<%=year %>-<%=month<10?"0"+month:month %>-01&dateEnd=<%=year %>-<%=month<10?"0"+month:month %>-<%=dayCount<10?"0"+dayCount:dayCount %>" target="_blank" ><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font></a><%} %></td>
			<%
				}
			%>
		<%}else{ %>
			<td <%if(i%2==0){ %>bgcolor="#eee9d9"<%}else{ %>bgcolor="#ffffff"<%} %>><%if(i!=statArray.length-1&&j==0){%><a href="sortingGroupStatAction.do?method=sortingGroupStatByHour&date=<%=statArray[i][j]==null?"0":statArray[i][j] %>" target="_blank"><%=statArray[i][j]==null?"0":statArray[i][j] %></a><%}else{%><%=statArray[i][j]==null?"0":statArray[i][j] %><%} %></td>
		<%} %>
	<%} %>
</tr>
<%} %>
