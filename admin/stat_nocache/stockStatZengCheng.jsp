		...
		<script type="text/javascript">
			...
			function forwardPage(type){
				...
				if(showTd!="undefind" && showTd!=null ){
					if(showTd.checked==true){
						if(type==1){
							document.location.href="stockStatZeng.do?p=<%=p-1%>&checkStr=1";
						}else if(type==2){
							document.location.href="stockStatZeng.do?p=<%=p+1%>&checkStr=1";
						}
					}else{
						if(type==1){
							document.location.href="stockStatZeng.do?p=<%=p-1%>";
						}else if(type==2){
							document.location.href="stockStatZeng.do?p=<%=p+1%>";
						}
					}
	...
	<%
		if(list!=null&& list.size()>0){
			for(int i=0;i<list.size();i++){
				...
				%>
				<tr bgcolor='#F8F8F8'>
					<td align="center" ><a href="searchorder.do?stockDate=<%=bean.getDateTime()%>&stockArea=3" target="_blank"><%=bean.getDateTime()%></a></td>
					<td align="center"><a href="searchorder.do?stockDate=<%=bean.getDateTime()%>&stockArea=3" target="_blank" ><%=sumNum%></a></td>
					<%if(group.isFlag(28)){%>
					<td align="center"><%=NumberUtil.priceOrder1(sumMoney)%></td>
					<%}%>
					<td align="center"><a href="searchorder.do?stockDate=<%=bean.getDateTime()%>&stockArea=3&orderStatus=11" target="_blank"><%=blackNum%></a></td>
					<%if(group.isFlag(28)){%>
					...
					for(int x=0;x<deliverList.size();x++){
						...
						for(int j =0;j<bean.getSubBeanList().size();j++){
							...
							if(deliverBean.getDeliver()==Integer.parseInt(deliverStr)){
								%>
					<td align="center" ><a href="searchorder.do?stockDate=<%=bean.getDateTime()%>&stockArea=3&deliver=<%=deliverBean.getDeliver()%>" target="_blank"><%=deliverBean.getSumNum()%></a></td>
					...
					<%}%>
					<td align ="center" ><a href="searchorder.do?stockDate=<%=bean.getDateTime()%>&stockArea=3&deliver=<%=deliverBean.getDeliver()%>&orderStatus=11" target="_blank"><%=deliverBean.getBackNum()%></a></td>
