...
function saveFun(){
	...
	var fittingCodes = [];
	var fittingCounts = [];
	var detectCode = $('#detectCode' ).val().trim();
	var flag = false;
	for(var i = 0; i < count; i++){
		...
		if(!flag){
			fittingCodes[fittingCodes.length] = $('#fittingCode_' + i).val().trim();
			fittingCounts[fittingCounts.length] = $('#count_' + i).numberbox('getValue').trim();
			flag = false
		}
	}
	$.ajax({
...
function submitFun(){
	...
	if(data != null){
		...
		$.ajax({
			url :'${pageContext.request.contextPath}/fittingController/addReceiveFitting.mmx',
			type : 'post',
			dataType : 'json',
			cache : false,
			asynchronous : false,
