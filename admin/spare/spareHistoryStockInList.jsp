...
$(function(){
	datagrid = $('#datagrid').datagrid({
	    ...
	    columns:[[  
	    	{field:'operateItemCode',title:'入库单号',width:30,align:'center'},
	        ...
	        {field:'createDatetime',title:'入库时间',width:50,align:'center',
	        	formatter : function(value,rowData,rowIndex){
	        		if(value != null){
	        			return value.substring(0,19);
	        		}
        		}}, 
	        {field:'operateUsername',title:'入库人',width:30,align:'center'},
	        {field:'typeName',title:'入库类型',width:30,align:'center'},  
	        {field:'action',title:'操作',width:30,align:'center',
	        	formatter : function(value, row, index) {
	        		if(row.type==1){
	        			return '<a href="javascript:void(0);"  class="query" onclick="querySpare('+index+',1)"></a>';
	        		}else if(row.type==2){
	        			return '<a href="javascript:void(0);"  class="query" onclick="querySpare('+index+',2)"></a>';
	        		}else if(row.type==5){
	        			return '<a href="javascript:void(0);"  class="query" onclick="querySpare('+index+',4)"></a>';
	        		}
				}}
	    ]],
	...
	$("#stockInOutType").combobox({
		url : '${pageContext.request.contextPath}/Combobox/getSpareStockCardType.mmx?flag=in',
		valueField:'id',
		textField:'text',
		delay:500
	});
