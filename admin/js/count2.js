...
$(document).ready(function () {
	var wordInput = $("#word");
	var value = document.all.proxytext.value=document.getElementById('proxy').options[document.getElementById('proxy').selectedIndex].text;
	wordInput.val(value);
		...
		if(wordtext!=""||myKeyCode==8||myKeyCode==46)
			{
				...
				$.post("/adult-admin/admin/autoProxyName.jsp", {word:wordtext}, function (date)
				 {
					var org = $(date);
					...
					wordnodes.each(function (i) 
					{
						...
						newDivNode.click(function()
						{
							...
							$("#auto").hide();
							selectOptions(document.getElementById('proxy'), $("#word").val());
...
function selectOptions(select,value){
	for(var i = 0; i < select.length; i++){
		if(document.getElementById('proxy').options[i].text == value){
