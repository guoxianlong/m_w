...
$(function(){
	datagrid = $('#detectTypeDetailManagerDataGrid').datagrid({
	    ...
	    columns : [[
			...
			{field:'content',title:'内容二级分类<input id=\"detectTypeDetailSecond\"/>',width:80,align:'center',editor : {type : 'text'}},
					...
					success : function(result) {
						try {
							...
							if (r.success) {
								dialog.dialog("close");
								datagrid.datagrid('reload');
								$('#detectTypeDetailSecond').combobox('reload');
							}
							$.messager.show({
								title : '提示',
								msg : decodeURI(r.msg)
	...
	$('#detectTypeDetailSecond').combobox({
...
function queryDetectTypeDetail(index,level){
	...
	detectTypeDetailId = row.id;
	if(level == 3){
		window.location.href = '${pageContext.request.contextPath}/admin/afStock/afterSaleDetectTypeDetailThirdManager.jsp?afterSaleDetectTypeId='
			+row.afterSaleDetectTypeId+"&parentId1="+row.parentId1+"&detectTypeParentId1="+row.detectTypeParentId1
			+"&detectTypeParentId2="+row.id+"&detectTypeDetailId="+row.id;
	}
}
...
function back(){
	window.location.href = '${pageContext.request.contextPath}/admin/afStock/afterSaleDetectTypeDetailFirstManager.jsp?afterSaleDetectTypeId='
		+detectTypeId+"&parentId1="+parentId1+"&detectTypeParentId1=0&detectTypeParentId2=0&detectTypeDetailId="+detectTypeDetailId;
}
...
<body>
	...
	<div id="tb"  style="height: auto;display: none;">
		<div align="right">
			...
			<input  type="button"  id="add" onclick="addDetectTypeDetail();" value="添加内容二级分类"/>
	...
	<div id="addContentDiv">
		<form id="addContentForm" method="post">
			<table id="addContentTable" class="tableForm" >
				...
				<tr>
					<td>
						添加内容二级分类：<textarea id='content' name='content' rows='10' cols='60'></textarea>
					</td>
				</tr>
			</table>
		</form>
