...
function checkDate(){
    ...
    if (!re.test(dateStart)&&dateStart!=""&&dateEnd!=""){
         alert('复核日期的起始日期，请输入正确的格式！如：2011-10-09');
         return false;
    }
    if (!re.test(dateEnd)&&dateStart!=""&&dateEnd!=""){
        alert('复核日期的截止日期，请输入正确的格式！如：2011-10-09');
        return false;
   }
	return true;
}
...
<body>
&nbsp;&nbsp;&nbsp;个人复核量统计
<form action="checkOrderStat.do?method=checkOrderStatByName" method="post" onsubmit="return checkDate();">
复核人：<input type="text" name="userName" maxlength="45" value="<%=userName %>"/>
...
<tr align="center" <%if(i==0||i==1){ %>bgcolor="#4688D6"<%} %>>
	<%for(int j=0;j<statArray[0].length;j++){ %>
		<%if(i==0){ %>
			...
			<%j++; %>
		<%}else if(i==1){ %>
			<td><font color="#FFFFFF"><%=statArray[i][j]==null?"0":statArray[i][j] %></font></td>
