...
function checkDate(){
    ...
    if (!re.test(date)){
         alert('复核日期，请输入正确的格式！如：2011-10-09');
         return false;
    }
	return true;
}
...
<body>
&nbsp;&nbsp;&nbsp;每小时复核量统计
<form action="checkOrderStat.do?method=checkOrderStatByHour" method="post" onsubmit="return checkDate();">
复核日期：
...
<tr align="center" <%if(i==0||i==1){ %>bgcolor="#4688D6"<%} %>>
	<%for(int j=0;j<statArray[0].length;j++){ %>
		<%if(i==0){ %>
			<td align="center" colspan="2"><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font></td>
			<%j++; %>
		<%}else if(i==1){ %>
