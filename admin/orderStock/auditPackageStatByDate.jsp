...
<body>
&nbsp;&nbsp;&nbsp;每日贴单量统计
<form action="auditPackageStat.do?method=auditPackageStatByDate" method="post">
贴单月份：
...
<tr align="center" <%if(i==0){ %>bgcolor="#4688D6"<%} %>>
	<%for(int j=0;j<statArray[0].length;j++){ %>
		<%if(i==0){ %>
			<td ><%if(j==0||j==(statArray[0].length-1)){ %><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font><%}else{ %><a href="auditPackageStat.do?method=auditPackageStatByName&userName=<%=statArray[i][j]==null?"":statArray[i][j] %>&dateStart=<%=year %>-<%=month<10?"0"+month:month %>-01&dateEnd=<%=year %>-<%=month<10?"0"+month:month %>-<%=dayCount<10?"0"+dayCount:dayCount %>" target="_blank" ><font color="#FFFFFF"><%=statArray[i][j]==null?"":statArray[i][j] %></font></a><%} %></td>
		<%}else{ %>
			<td <%if(i%2==0){ %>bgcolor="#eee9d9"<%}else{ %>bgcolor="#ffffff"<%} %>><%if(i!=statArray.length-1&&j==0){%><a href="auditPackageStat.do?method=auditPackageStatByHour&date=<%=statArray[i][j]==null?"0":statArray[i][j] %>" target="_blank"><%=statArray[i][j]==null?"0":statArray[i][j] %></a><%}else{%><%=statArray[i][j]==null?"0":statArray[i][j] %><%} %></td>
		<%} %>
	<%} %>
</tr>
<%} %>
