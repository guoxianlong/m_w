...
<script type="text/javascript">
	...
	function openPrintPage(userCode,sortingBatchGroupId,printType,selectedOrder,sortingType){
		 if( window.confirm("是否继续操作")){
			 window.location.href = 'sortingAction.do?method=sortingBatchGroupPrintLine&userCode='+userCode+'&sortingBatchGroupId='+sortingBatchGroupId+'&printType='+printType+'&selectedOrder='+selectedOrder+'&pageFrom=sortingBatchOrderReceive&type='+sortingType+'&success=1&staffCode=<%=staffCode%>&sortingBatchGroupId=<%=id%>';
			 return true;	 
		 }else{
			 return false;
		 }
...
<body onload="load()">
<div align="left"><font size="7" color="red">PDA分拣波次领取</font></div>
<hr>
<table border="0" >
	<tr>
		<td><%if(ranking!=null){ %>
...
<form method="post" action="<%=request.getContextPath()+"/admin/sortingAction.do?method=sortingBatchOrderReceive&type=pda" %>">
	<table>
		<tr>
...
<%if(("1").equals(success)) {%>
&nbsp;&nbsp;&nbsp;<%=staffBean.getName() %>(<%=staffCode%>),波次号:<%=groupBean.getCode()%>领取成功!&nbsp;&nbsp;&nbsp;<input onClick="openPrintPage('',<%=groupBean.getId()%>,'buda','','<%=sortingType %>')" style="color:blue; font-size:14px" type="button" value="补打">
