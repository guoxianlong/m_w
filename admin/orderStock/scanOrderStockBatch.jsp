 ...
 *批量验证imei码
 */
function batchCheckIMEI(){
	var imeiCodes = document.getElementById("imeiCode").value;
	$.ajax({
	 	type: "post",
	 	url: '<%=request.getContextPath()%>/admin/orderStockAction.do?method=batchCheckIMEI',
	 	cache: false,
	 	dataType: "json",
	 	data: {imeiCodes: imeiCodes},
	 	success: function(r){
	 		if(r.success){
				var map = r.obj;	 			
	 			for (var key in map) {
		 			if(!checkProduct(map[key],key)){
		 				return;
		 			}
	 	        }
 ...
 * 验证IMEI码
 */
function checkIMEI(productCode){
	...
	if(<%=flag%>){
		$.ajax({
		 	...
		 	success: function(r){
		 		if(r.success){
			 		checkProduct(r.obj.productCode,r.obj.imeiId);
			 		return true;
		 		} else {
		 			alert(r.msg);
		 			return false;
...
function checkProduct(productCodeValue,imeiId){
			...
			if(isProduct(tempCode,productCodeValue)){
				...
				if(checkNum){
					...
					if(sumNum><%=tmpsh.getStockoutCount()%>){
						customAlert("<%=tmpsh.getProduct().getOriname().replace("\r","").replace("\n", "").replace("\"","“").replace("\'","‘")%> 的扫描量<font color='red'>"+sumNum+"</font>超出实际应复核量<font color='red'><%=tmpsh.getStockoutCount()%></font>，请重新核实！");
						onLoadMe();
						return false;	
					}else{
						// 移入焦点
...
function updateCheckNum(procNumId,tmpBatchBarcode){
	//alert(checkIMEI(tmpBatchBarcode));	
	if(!checkIMEI(tmpBatchBarcode)){
		return;
	}
	...
	if(enterNumObj){
		...
		if(enterNumObj.value.length==0 || !reg.test(enterNumObj.value)){
			...
			if(procNumObj.value>stockOutCount){
				customAlert(oriname+" 的扫描量<font color='red'>"+procNumObj.value+"</font>超出实际应复核量<font color='red'>"+stockOutCount+"</font>，请重新核实！");
				onLoadMe();
			}
		}
	}
...
地区：<select name="area" <%= (!changeArea && (bean.getStatus() == OrderStockBean.STATUS1 || bean.getStatus() == OrderStockBean.STATUS2))?"disabled=\"disabled\"":"" %> >
<option value="1" <%if(bean.getStockArea() == 1){%>selected<%}%>>广分</option>
<option value="2" <%if(bean.getStockArea() == 2){%>selected<%}%>>广速</option></select>
...
注：在已完成状态下地区将不被更改。&nbsp;&nbsp;
<%if(group.isFlag(949)){ %>
批量复核：<textarea id="imeiCode" name="imeiCode" cols="30" rows="5" ></textarea>
<input type="button" value="提交" onclick="batchCheckIMEI()">
<%} %>
...
<fieldset>
	<legend>扫描复核区</legend>
		扫描产品条码或输入产品编号：<input type="text" maxlength="30" name="productCode" id="productCode" size="25"  onkeyup="onKeyDown(event);"> 
