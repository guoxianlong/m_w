...
<form method=post action="updateBuyOrderSupplier.jsp" name="importForm">
<table width="30%" cellpadding="3" cellspacing="1" bgcolor="#e8e8e8" align=center>
	<tr bgcolor='#F8F8F8'>
		<td align=left>
		前缀：<input type="text" name="aCode"/>&nbsp;&nbsp;库类型：<input type="text" name="stockType"/>&nbsp;&nbsp;
		货架层数：<input type="text" name="floorCount"/>&nbsp;&nbsp;库区域ID：<input type="text" name="stockAreaId"/>&nbsp;&nbsp;
		仓库ID：<input type="text" name="storageId"/>&nbsp;&nbsp;地区ID：<input type="text" name="areaId"/>&nbsp;&nbsp;
		城市ID：<input type="text" name="cityId"/>&nbsp;&nbsp;添加货架个数：<input type="text" name="count"/>&nbsp;&nbsp;
...
if(!type.equals("shelf")){
	...
	try{
		//根据库区域查询所有甬道
		List passageList = service.getCargoInfoPassageList("stock_area_id = "+stockAreaId,-1,-1,"id asc");
