...
<script language="JavaScript" >
	...
	function submit(){
		...
		List<ProductStockBean> list = insertAction.productLockCountStatistics(request,response);
		%>
	}
</script>
...
<font size="6" color="red">共检索<%=list.get(0).getId() %>条记录 ,查询数据库<%=(list.get(0).getId())*5+2 %>次,用时<%=list.get(0).getStatus()/1000%>秒,有<%=list.size() %>条符合要求的记录</font>
<table  width="70%" align="center">
	<tr>
		<td width="">商品编号</td><td width="">货位编号</td><td  width="">货位库存锁定量</td><td  width="">其他单子锁定量</td><td  width="">差值</td><td  width="">库类型</td><td width="">库地区</td>
	</tr>
	<%for(ProductStockBean bean : list){ %>
	<tr>
		<td><%=bean.getProductId() %></td><td><%=bean.getWhole_code()%></td><td><%=bean.getStock() %></td>
		<td><%=bean.getLockCount() %></td><td><%=bean.getAllStock() %></td>
