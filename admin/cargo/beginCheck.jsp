  ...
  $("#productCode").keydown(function(e){  
    if(e.keyCode==13){   
        ...
        if(productCode=="KAI"){//扫描开始码
        	...
        	window.location.href="beginCheck.mmx?productCode="+productCode+"&batchId="+batchId+"&missionId="+missionId+"&buyStockinCode="+buyStockinCode;
        }
    		...
    		validProduct(productCode,csmcode);
	    		...
	    		$("#productCode").val("");
	    		$.post('checkStatus.mmx',{batchId:batchId,type:2},function(result){
	    		var rs = result.split(",");
	    		if(rs[0]=="0"){//先录入的是初录
		    		$.post('updateFristCheck.mmx',{csmCode:csmcode,productCount:productCount,batchId:batchId,type:2},function(msg){
		    		    
		    			var msg1 = msg.split(",")[0];
						 if(msg1=="1"){
						 	 $("#result").html("初录成功!");
						 }else if(msg1=="-1"){
						 	alert("采购已完成，不允许执行入库操作!");
						 }else{
						 	alert("初录失败!");
						 }
						 $("#productCode").val("");
