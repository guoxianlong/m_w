...
<%@page import="adultadmin.action.vo.voUser,mmb.stock.stat.*"%>
<%
...
String  area = (String) request.getAttribute("areaId");
...
int ai = StringUtil.toInt(area);
String wareAreaLable = ProductWarePropertyService.getWeraAreaOptionsCustomized("areaId", "areaId", request,ai,true,"-1");
%>
...
<html>
  ...
  <body>
  	<form action="<%=basePath%>admin/cargoUpOper.do?method=shelfUpList" method="post"> 
	  	 产品上架作业单<br/>
	  	<fieldset style="width:850px;"><legend>查询栏</legend>
			...
			作业单状态:<input type="checkBox" name="status" id="status" value="1"/>未处理 &nbsp; 
					 <input type="checkBox" name="status" id="status" value="2"/>提交并确认 &nbsp;
			         <input type="checkBox" name="status" id="status" value="3"/>交接阶段 &nbsp;
			         <input type="checkBox" name="status" id="status" value="7"/>作业结束 &nbsp;
					 <br/>
			...
			货位号:<input type="text" size="15" name="cargoCode" value="<%=cargoCode%>"/>精确&nbsp;&nbsp;
			库地区：<%= wareAreaLable%>&nbsp;&nbsp;
			制单人:<input type="text" size="15" name="userName" value=""/>(用户名)&nbsp;&nbsp;<br/>
			调拨单（汇总单）编号:<input type="text" size="15" name="exChangeCode" id="exChangeCode" value="<%=exChangeCode%>"/>&nbsp;&nbsp;&nbsp; 
			<input type="submit" value="查询"/> &nbsp;&nbsp;&nbsp;
			<a href="<%=basePath%>admin/cargoUpOper.do?method=showExChangeList">转换新的上架作业单</a>		  
	  	</fieldset>	
	</form>	 	
	...
	<table cellpadding="3" border=1 style="border-collapse:collapse;" bordercolor="#D8D8D5" width="95%" >
		<tr bgcolor="#4688D6">
			...
			<td align="center"><font color="#FFFFFF">目的仓库</font></td>
		...
		%>
			<tr <%if(i%2==0){ %>bgcolor="#eee9d9"<%}else{ %>bgcolor="#ffffff"<%} %>>
				...
				<td align="center"><a href="<%=basePath%>admin/cargoOper.do?method=showEditCargoOperation&operationId=<%=bean.getId()%>"><%=bean.getCode()%></a></td>
				...
				<td align="left" >
					<a href="<%=basePath%>admin/cargoOper.do?method=showEditCargoOperation&operationId=<%=bean.getId()%>">编辑</a>&nbsp;
					<%if(bean.getStatus() == CargoOperationProcessBean.OPERATION_STATUS1&&bean.getCreateUserId()==user.getId()){ %>
						<a href="<%=basePath%>admin/cargoOper.do?method=delCargoOperation&operationId=<%=bean.getId()%><%=para %>&pageIndex=<%=request.getParameter("pageIndex")%>" onclick="javascript:return confirm('如果确认删除，请单击‘确定’，反之，请单击‘取消’！')">删除</a>
					<%} %>
					|<a href="admin/cargoOperation.do?method=printUpShelfCargo&id=<%=bean.getId() %>" target="_blank">打印作业单<%if(bean.getPrintCount()>0){ %>(<%=bean.getPrintCount() %>)<%} %></a>
				</td>
				<td align="center" >
				<%if(bean.getExChnageId() ==0){%>
					<%=bean.getSource()%>
				<%}else{%>
					<a href="<%=basePath%>admin/productStock/stockExchange.jsp?exchangeId=<%=bean.getExChnageId()%>" target="_blank"><%=bean.getSource()%></a>
				<%}%>
				</td> 
			</tr>
		<% }
