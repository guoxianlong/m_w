...
function checkButton(){
	...
	if(cartonningCode != "" && cartonningCode != "扫描装箱单编号"){
		...
		setTimeout("window.location='<%=request.getContextPath()%>/admin/cargo/soDropCartonning.jsp';",5000);
	}else{
		alert("装箱单编号不能为空！");
...
<tr height="4">
	<td colspan="2" align="center"><font size="4"style="font-weight:bold">作废装箱单</font>
...
<form action="<%=request.getContextPath()%>/admin/stockOperation.do?method=dropCartonning" method="post">
<% if(result != null){ %>
	<tr height="28">
...
<% }else{ %>
	<tr height="28">
		<td colspan="2" align="center"> <textarea cols="26" rows="7" name="" readonly="true" style="overflow-x:hidden;overflow-y:hidden">请扫描要作废的装箱单号.</textarea></td>
	</tr>
<% } %>
	<tr height="8">
		<td colspan="2" align="center">
		<input type="text" size="32" id="cartonningCode" name="cartonningCode"onblur="prompt()" onfocus="clearText()" onkeypress="javascript:return checkEnter()">
		</td>	
	</tr>
	...
	<tr height="8" align="center">
		<td>
		<input type="button" value="作废装箱单"  style="height:26px;width:130px;"onclick="checkButton()"/>
		<input type="button" value="返   回" style="height:26px;width:80px" onclick="window.location='<%=request.getContextPath()%>/admin/stockOperation.do?method=stockOperation&toPage=zhuangxiangguanli'" />
		</td>
	</tr>
</form>
</table>
