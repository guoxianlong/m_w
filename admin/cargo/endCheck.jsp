  ...
  $("#productCode").keydown(function(e){  
    if(e.keyCode==13){   
        ...
        if(productCode=="KAI"){//扫描开始码
        	...
        	window.location.href="endCheck.mmx?productCode="+productCode+"&batchId="+batchId+"&missionId="+missionId+"&buyStockinCode="+buyStockinCode;
        }
    	...
    	if($("#productCount").val()==""){
	    	...
	    	if(productCount==0){
	    		...
	    		$("#productCode").val("");
	    	$.post('checkStatus.mmx',{batchId:batchId,type:1},function(result){
	    	var rs = result.split(",");
	    		if(rs[0]=="1"){
							    	...
							    	autocomplete(buyStockinCode);
							 		...
							 		var t = window.showModalDialog("cargo/valComplete.jsp","请确认","dialogWidth:200px;status=no;dialogHeight:150px;help=no;location=no;resizable");							 	
									    	if(t){
									    		...
									    		autocomplete(buyStockinCode);
									    		window.location.href="editCheckStockinMission.mmx?missionId="+missionId;
									    	}								    	
									    }
							 });
						    	...
						    	$("#fristStatus").val(0);
				    	   	  ...
				    	   	  for(var i=2;i<msg4.length-1;i++){
				    	   	  	codelist = codelist+msg4[i].split("-")[1]+",";			    	   	  		
				    	   	  		 //window.location.href="printCartonningInfo1.mmx?code="+msg4[i].split("-")[1]+"&missionId="+missionId+"&batchId="+batchId;
								 }
								 window.open("printCartonningInfo1.mmx?code="+codelist+"&missionId="+missionId+"&batchId="+batchId,'打印装箱单','height=100,width=400,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
...
function changeBinning(){
	...
	$.post('changeBinning.mmx',{binning:binning,productId:productId},function(result){
		if(result=="-1"){
...
function validProduct(obj,csmcode){
	    	...
	    	$.post('validProduct.mmx',{productCode:obj,csmCode:csmcode},function(msg){
	    	var productId=$("#productId").val();
	    		if(msg=="-1"){
...
function validComplete(missionid){
	var batchId=$("#batchId").val();
	var missionId=$("#missionId").val();
	$.post('validComplete.mmx',{missionId:missionid},function(msg){
		if(msg=="0"){
...
function init(){
	var batchId = $("#batchId").val();
	$.post('initLeftCount.mmx',{batchId:batchId},function(result){
	    if(result=="-1")
	    	alert("未找到对应的数据!");
...
function autocomplete(obj){
	$.post('autocompleteorder.mmx',{buyStockinCode:obj},function(result){
		if(result=="-1"){
...
<center>
	...
	<div class="easyui-panel" title="" style="width:400px">
	    ...
	    <form id="ff" method="post" action="updateFristCheck.mmx">
	    	<table>
	    		<tr>
	    			<td><h3>预计单号:</h3></td>
