...
function checkSubmit(){
	...
	for(var i=0;i<cargoId.length;i++){
		if(cargoId[i].checked ==true){
			if(confirm("如果确认开通，请单击‘确定’，反之，请单击‘取消’！")==false){
				return false;
			}else{
				return true;
			}
...
<body>
<form action="../admin/cargoInfo.do?method=openCargoList" method="post">
批量开通货位（仅查询未开通货位）<br/>
<fieldset style="width:880px;"><legend>查询栏</legend>
...
库存类型：<select name="stockType">
			...
			<option value=11 <%if(stockType!=null&&stockType.equals("11")){ %>selected=selected<%} %>>配件售后库</option>
		</select>&nbsp;&nbsp;&nbsp;&nbsp;
...
<form action="../admin/cargoInfo.do?method=openCargo" method="post">
...
<input type="submit" onclick="return checkSubmit()" value="批量开通货位" style="MARGIN-LEFT: 10px"/>
</form>
<%if(paging!=null){ %>
