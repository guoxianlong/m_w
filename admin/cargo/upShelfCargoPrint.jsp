...
<object id="LODOP" classid="clsid:2105C259-1E0C-4534-8141-A753534CB4CA" width=0 height=0> 
	...
	for(int i=0;i<cocList.size();i++){ 
		CargoOperationCargoBean outCoc = (CargoOperationCargoBean)cocList.get(i);
		List inCocList=(List)outCoc.getCocList(); 
		for(int j=0;j<inCocList.size();j++){
			...
			%>
				...
				<tr>
					<td colspan="3"  style="border-right:0;height:60px; text-align:center; vertical-align: middle;"><h2>物流上架单</h2></td>
			...
			<%	
				...
				<tr>
					...
					<td align="center"><b><%=inCoc.getStockCount() %></b></td>
					<td><b><%=inCoc.getCargoInfo().getWholeCode() %></b></td>
				</tr>
			<%
			count++;
			totalStockCount = totalStockCount + inCoc.getStockCount();
...
while(iter.hasNext()){
	...
	%>
		<div align="center" id="printDiv<%=index%>">
			<table cellpadding="0" cellspacing="0" width="570" height="400" border="1" >
				<tr>
					<td colspan="3"  style="border-right:0;height:60px; text-align:center; vertical-align: middle;"><span style="font-size:24;font-weight:bold;">物流上架单</span><br/>（调拨单号：<%=cargoOperation.getSource() %>）
					</td>
					<td height="60" align="right" colspan="3" id="barcodeID<%=index %>" style="border-left: 0;"><div id="barcodeImage<%=index%>"><%=cargoOperation.getCode() %></div></td>
				</tr>
				<tr>
