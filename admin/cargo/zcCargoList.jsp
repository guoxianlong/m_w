...
<body>
	<%=ProductStockBean.areaMap.get(Integer.valueOf(areaId)) %>货位列表
	<form action="../admin/cargoInfo.do?method=zcCargoList" method="post">
		<fieldset style="width:780px;"><legend>查询栏</legend>
		货&nbsp;&nbsp;位&nbsp;号：<input type="text" size=15 name="wholeCode" <%if(wholeCode!=null){ %>value="<%=wholeCode%>"<%}%> />左精确右模糊查询&nbsp;&nbsp;&nbsp;&nbsp;
		...
		存放类型：<select name="storeType">
					...
					<option value="6" <%if(storeType!=null&&storeType.equals("6")){ %>selected=selected<%} %>>包裹区</option>
					<option value="7" <%if(storeType!=null&&storeType.equals("7")){ %>selected=selected<%} %>>下架区</option>
				</select>&nbsp;&nbsp;&nbsp;&nbsp;
		...
		仓库类型：<select name="stockType">
					<option value="">请选择</option>
					<%for (int i : ProductStockBean.stockTypeMap.keySet()) { %>
						<option value=<%=i %> <%if(stockType == i){ %>selected=selected<%} %>><%=ProductStockBean.stockTypeMap.get(i) %></option>
					<%} %>
		         </select>
		         <br/>
		...
		货位当前库存：<input type="text" name="stockCountStart" size=3 <%if(stockCountStart>=0){ %>value="<%=stockCountStart%>"<%} %>/>
						至<input type="text" name="stockCountEnd" size=3 <%if(stockCountEnd>=0){ %>value="<%=stockCountEnd%>"<%} %>/><br/>
		...
		货位状态：<input type="checkbox" name="status" value="0" />使用中
				...
				<input type="checkbox" name="status" value="2" />未开通&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="hidden" value="<%=areaId %>" name="areaId"/><br>
	...
	<table cellpadding="3" border=1 style="border-collapse:collapse;" bordercolor="#D8D8D5">
		...
		<%for(int i=0;i<list.size();i++){ %>
			<tr <%if(i%2==0){ %>bgcolor="#eee9d9"<%}else{ %>bgcolor="#ffffff"<%} %>>
				...
				<td><%=bean.getStockCount()+bean.getStockLockCount() %>(<a href="cargoOperation.do?method=stockExchangeList&cargoWholeCode=<%=bean.getCargoInfo().getWholeCode() %>&productCode=<%=bean.getProduct().getCode() %>&type=stockLock" target="_blank"><%=bean.getStockLockCount() %></a>)</td>
				<td>
