...
<%@ page import="java.util.List,java.util.ArrayList,adultadmin.util.PageUtil,adultadmin.bean.PagingBean " %>
<%@ page import="adultadmin.bean.cargo.*" %>
<%@ page import="adultadmin.action.cargo.*" %>
<%@ page import="org.apache.struts.action.*" %>
<%@ page import="adultadmin.action.vo.*,adultadmin.util.*" %>
<html>
<head>
<title>货位绑定产品——查询货位</title>

<%
List inCargoBeanList=new ArrayList();
...
function submitCargo(){
	var selected=false;
	for(var i=0;i<document.getElementsByName("inCargoId").length;i++){
		var product=document.getElementsByName("inCargoId")[i];
		if(product.checked==true){
			selected=true;
			window.opener.document.getElementById("cargoCode").value=document.getElementsByName("inCargoId")[i].value;
			if(confirm("选择了货位："+document.getElementsByName("inCargoId")[i].value+"\r\r"+"单击“确定”将选中该货位并关闭本页面  ")){
				window.close();
			}
		}
	}
...
<body>
	...
	<p>查询货位</p>
	<form action="../admin/cargoInfo.do?method=selectCargo" method="post">
		...
		货位状态：<select name="status">
					...
					<option value=1 <%if(request.getParameter("status")!=null&&request.getParameter("status").equals("1")){ %>selected=selected<%}else if(request.getParameter("status")==null){ %>selected=selected<%} %>>未使用</option>
				</select><br/>
	...
	<table cellpadding="3" cellspacing="1" border=1 >
		...
		<%for(int i=0;i<inCargoBeanList.size();i++){ %>
			<tr>
				...
				<td><input type="radio" id="inCargoId" name="inCargoId" value="<%=ciBean.getWholeCode() %>"/></td>
	...
	<input type="button" onclick="return submitCargo();" value="确定"/>
	</form>
	<%if(paging!=null){ %>
