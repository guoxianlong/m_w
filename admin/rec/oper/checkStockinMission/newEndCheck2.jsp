...
<%
	int firstStatus = (Integer)request.getAttribute("firstStatus");
	int secondStatus = (Integer) request.getAttribute("secondStatus");
%>
<%
...
<body onload="$('#buyStockCode').attr('disabled',true);$('#productCode').focus();">
	<center>
		<h3>质检结果复录录入</h3>
		<div class="easyui-panel" style="width:500px;" align="center">
			...
			<table>
				<tr>
					...
					<input id="buyStockCode" name="buyStockCode" class="easyui-validatebox" value="${buyStockinCode}"/>
					</form>
					</td>
					<td align="left"><span id="buyStockInfo">入库任务单号：<a href='<%= request.getContextPath()%>/admin/editCheckStockinMission.mmx?missionId=${missionId}' target='_blank' >${buyStockCode}</a></span></td>
				</tr>
				<tr>
					<td align="left">
					<b>SKU:</b> 
			...
			<div id="changeBinning" style="display:block;">
				商品编号: <span id="binningProductCode"></span>
						标准装箱量:<input id="binn" name="binn" class="easyui-numberbox" required value="${binning}" style="width:40px">
						<a class="easyui-linkbutton" href="javascript:void(0)" onclick="changeBinning();">确定</a>
						<span id="changeResult" name="changeResult"></span>
			</div>
			<input type="hidden" name="hideBuyStockCode" id="hideBuyStockCode" value="${buyStockinCode}" />
			<input type="hidden" name="hideProductCode" id="hideProductCode" value="${productCode}" />
			<input type="hidden" name="hideProductId" id="hideProductId" value="${productId}" />
			<input type="hidden" name="hideCheckStockinMissionId" id="hideCheckStockinMissionId" value="${missionId}" />
			<input type="hidden" name="hideCheckStockinMissionBatchId" id="hideCheckStockinMissionBatchId" value="${batchId}" />
			<input type="hidden" name="hideProductBarCode" id="hideProductBarCode" value="${productBarCode}" />
		...
		<input type="hidden" id="checkStockinMissionMark" value="1"  />
	<script type="text/javascript">
		...
		function checkProductCodeAndGetCheckStockinInfo() {
			...
			if( checkStockinMissionMark == "0" ) {
				...
				var hideProductBarCode = $("#hideProductBarCode").val();
				hideProductBarCode = hideProductBarCode.trim();
				if( hideProductCode == "" ) {
					$("#tipInfo").css("background","#FF0000");
						Play('<%= soundSrcSJYC%>');
					...
					function (data,textStatus) {
						if ( data['status'] == "success" ) {
							...
							$("#tipInfo").html(sht);
							 //入库成功才进行判断
							 $.post('<%= request.getContextPath()%>/admin/validComplete2.mmx',{missionId:missionId},function(msg1){
							    if(msg1=="1"){
					...
					$.messager.alert("提示", "第" + tempCountError + "个商品不属于该任务单!",'error', function () {
						...
						$("#tipInfo").html(sht);
				} else if ( productCode == hideProductCode || productCode == hideProductBarCode) {
		            $("#tipInfo").css("background","green");
