...
$(function() {
	...
	$('#wxoutwaitExchangeDataGrid').datagrid({
		...
		toolbar : [{
			...
			iconCls : "icon-save",
			handler: function(){
				var checkedrows = $("#wxoutwaitExchangeDataGrid").datagrid("getChecked");
				if (checkedrows.length <= 0) {
					...
					var wxoutform = $('#wxoutform').form({
						url : '${pageContext.request.contextPath}/AreaStockExchangeController/exportExchangeProduct.mmx'
					});
					$("#wxoutform [id=type]").attr("value", "0");
					$("#wxoutform [id=flag]").attr("value", "1");
					$("#wxoutform [id=aseBeanId]").attr("value", ids);
					wxoutform.submit();
				}
			}
		}],
...
<table id="wxoutwaitExchangeDataGrid"></table>
<form id = "wxoutform">
