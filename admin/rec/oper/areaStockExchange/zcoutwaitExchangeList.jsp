...
$(function() {
	...
	$('#zcoutwaitExchangeDataGrid').datagrid({
		...
		toolbar : [{
			...
			iconCls : "icon-save",
			handler: function(){
				var checkedrows = $("#zcoutwaitExchangeDataGrid").datagrid("getChecked");
				if (checkedrows.length <= 0) {
					...
					var zcoutform = $('#zcoutform').form({
						url : '${pageContext.request.contextPath}/AreaStockExchangeController/exportExchangeProduct.mmx'
					});
					$("#zcoutform [id=type]").attr("value", "1");
					$("#zcoutform [id=flag]").attr("value", "1");
					$("#zcoutform [id=aseBeanId]").attr("value", ids);
					zcoutform.submit();
				}
			}
		}],
...
<table id="zcoutwaitExchangeDataGrid"></table>
<form id = "zcoutform">
