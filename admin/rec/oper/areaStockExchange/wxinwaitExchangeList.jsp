...
$(function() {
	...
	$('#wxinwaitExchangeDataGrid').datagrid({
		...
		toolbar : [{
			...
			iconCls : "icon-save",
			handler: function(){
				var checkedrows = $("#wxinwaitExchangeDataGrid").datagrid("getChecked");
				if (checkedrows.length <= 0) {
					...
					var wxinform = $('#wxinform').form({
						url : '${pageContext.request.contextPath}/AreaStockExchangeController/exportExchangeProduct.mmx'
					});
					$("#wxinform [id=type]").attr("value", "0");
					$("#wxinform [id=flag]").attr("value", "0");
					$("#wxinform [id=aseBeanId]").attr("value", ids);
					wxinform.submit();
				}
			}
		}],
...
<table id="wxinwaitExchangeDataGrid"></table>
<form id = "wxinform">
