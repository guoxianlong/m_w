...
<%
	...
	String deliver = (String)request.getAttribute("deliver");
	...
	String sortingBatchId = (String)request.getAttribute("sortingBatchId");
	OrderCustomerBean ocBean=(OrderCustomerBean)request.getAttribute("ocBean");
	Map productCountMap = (HashMap)request.getAttribute("productCountMap");//每个订单对应的商品总数量
	if (null != tip) {
	%>
		<script>
		alert("<%=tip%>");
...
<%
	...
	while (orderListIter.hasNext()) {//循环订单
		...
		<div id="tableDiv<%=bianjie%>">
			<table cellpadding="0" cellspacing="0" width="700" height="450" border="1" style="border: 1px solid; border-collapse: collapse; font-size: 12px;">
				<tr height="60px">
						<td colspan="6">
							<table width="694" cellpadding="1" style="border: none; border-collapse: collapse;">
								<tr>
									...
									<td id="barcodeID<%=bianjie%>" width=200px rowspan="2" align="right" style="height: 60px; text-align: center;">
										<div id="barcodeImage<%=bianjie%>"><%=order.getCode()%></div>
									</td>
									<td rowspan="2" align="right" > 
										<%--<span style="font-size:60px;font-weight:bold;"><%=SortingBatchBean.getCircleNumber(order.getSerialNumber())%></span>--%>
										<span style="font-size:15px;font-weight:bold;"><font style="font-size: 28px;"><%=index/6>0?"续":"" %></font></span>
									</td>
								</tr>
								<tr>
				...
				<tr>
						...
						序号:<%=order.getBatchNum()%>-<%=order.getSerialNumber() %>
						</td>
				...
				<tr>
					<td></td>
					<td align="left" style="font-size: 13px;" colspan="5"><%=orderProduct.getOriname()%></td>
				</tr>
				<%
				}else{ //非一个发货单的开头
				%>
				...
				<%}index++;
					if(index>6 && index%6==1){
						batchCode+=order.getBatchNum()+"-"+order.getSerialNumber()+";";
					}
					if(index%6==0){ 
				...
				<tr>
					<td colspan="2" align="left"><%if(productList.size()>6){ %>小计：<%=totalCount%>&nbsp;<%} %>商品总数：<%=productCountMap.get(order.getOrderStock().getId()+"")%></td>
		...
		<br />
				...
				<br />
					...
					<input type="button" value="确认打印" onclick='this.disabled="disabled";initPrintOrder()'>
					<a href='<%=request.getContextPath()%>/admin/sortingAction.do?method=orderStockPrint&deliver=<%=deliver %>&batchId=<%=sortingBatchId%>'><font color='blue'>导出</font></a> <br>一共&nbsp;<%=orderList.size() %>&nbsp;个订单&nbsp;其中有分页的：&nbsp;<%=batchCode %>
 ...
 <%--<%
	...
	int charNumPerLine = 40;//每行最多40个字符串，超出会换行
			...
			<table  width="740" >
				<tr>
					<td colspan="4" style="TEXT-ALIGN: center">
						...
						分拣波次汇总单(<%=sortingBatchGroupInfo.getDeliverName()%>)</SPAN></td>
				...
				<tr>
					...
					<td >商品数:<%=sortingBatchGroupInfo.getProductCount()%></td>
		  ...
		  <td>
			<table width="740" height="850"  cellpadding="0" cellspacing="0"  border="1" style="border: 1px ;  table-layout:fixed">
				<tr>
					...
					<td style="font-size:12px;width:120px;text-align:center;">产品编号</td>
	...
	<%
	if(SKUIndex>=huizongList.size()) break;
	}
}
%>--%>
...
<%if(orderList.size() > 0 && huizongList.size()>0&& huizongNum.size()>0){ 
	...
	alert("SKU数为0，无需分拣！");
	<%}%>
	<%if("sortingBatchOrderReceive".equals(pageFrom)){%>
		window.location.href="sortingAction.do?method=sortingBatchOrderReceiveInfo&success=1&staffCode=<%=staffCode%>&sortingBatchGroupId=<%=sortingBatchGroupInfo.getId()%>";
	<%}else{%>
