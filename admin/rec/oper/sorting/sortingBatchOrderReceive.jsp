...
<script type="text/javascript">
	...
	function openPrintPage(userCode,sortingBatchGroupId,printType,selectedOrder){
		 if( window.confirm("是否继续操作")){
			 window.location.href = '${pageContext.request.contextPath}/SortingController/sortingBatchGroupPrintLine.mmx?pageFrom=sortingBatchOrderReceive&userCode='+userCode+'&sortingBatchGroupId='+sortingBatchGroupId+'&printType='+printType+'&selectedOrder='+selectedOrder+'&pageIndex=<%=StringUtil.StringToId(request.getParameter("pageIndex"))%>';
			 return true;	 
		 }else{
			 return false;
		 }
...
<table border="0" >
	<tr>
		<td><%if(ranking!=null){ %>
		&nbsp;&nbsp;&nbsp;<font color="red" size="4">你目前已领取<%=oneselfCount %>个,<%=ranking %>,冠军<%if(photoUrl != null){ %><img alt="" width="83" height="100" src="<%=request.getContextPath() %>/upload/<%=photoUrl%>"/><%}%><%=firstCount %>个,继续加油哦~</font><br><br>
		<%} %>	&nbsp;&nbsp;&nbsp;<font size='3'>今日已分配分拣波次数：</font><font color='blue' size='3'><%=assignedCount1 %></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br><br>
		 ...
		 <%if(group.isFlag(644)){ %> <a href='${pageContext.request.contextPath}/admin/rec/oper/sorting/sortingStatistics.jsp'><font color='blue' size='3'>【分拣统计量】</font></a><%} %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		 <%if(group.isFlag(643)){ %> <a href='${pageContext.request.contextPath}/admin/rec/oper/sorting/sortingBatchGroupPrintList.jsp'><font color='red' size='3'>【分拣波次补打】</font></a><%} %>
		</td>
	</tr>
</table>
<form method="post" action="${pageContext.request.contextPath}/SortingController/sortingBatchOrderReceive.mmx">
	<table>
		<tr>
