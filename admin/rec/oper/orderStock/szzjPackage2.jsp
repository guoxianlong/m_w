...
<table id="__01"  border="1" cellpadding="0" cellspacing="0" style="font-family: SimHei;">
	...
	<tr>
		<td colspan="5" style="width: 100mm;height: 18mm;" align="left" >
			<img src="<%=request.getContextPath()%>/image/logo_fhd.bmp" width="130px" height="54px"/></td>
	</tr>
	<tr>
		<td colspan="5" style="height: 7mm"><strong>配送员存联</strong>&nbsp;&nbsp;&nbsp;&nbsp;快递公司：<%=order.getDeliverName() %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		 <font ><%if(order.getBuyMode()==0){%><strong>代收货款</strong><%}else {%><strong>已付款</strong><%}%></font></td>
	</tr>
	<tr>
		...
		<td colspan="1" rowspan="1" style="width: 47mm; height: 17mm;">
			<%if(order.getBuyMode()==0){%>代收金额：<%=order.getDprice() %>元<%}else{%>已付款<%} %><br/>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan="3" >
			地址：<font style="font-size: 20px;font-weight: bold;">买卖宝<%=ProductStockBean.areaMap.get(osBean.getStockArea()) %>仓</font><br>
			
			发货人:无锡买卖宝&nbsp;&nbsp;&nbsp;&nbsp;电话：40088-43211
			</td>
	...
	<tr>
		<td align="left" colspan="4" style="width: 65mm; height: 10mm;">
			订单:</td>
		<td style="width: 23mm; height: 10mm;text-align: center;">
	...
	<tr>
			<td colspan="3" style="height: 8mm;">姓名:<%=order.getName() %>&nbsp;&nbsp;&nbsp;&nbsp;邮编：<%=order.getPostcode() %><br>
			电话:<%=order.getPhone() %></td>
...
function initPrint(){
	...
	if(LODOP.PRINT_INIT("")){ //mmb发货清单打印
		...
		LODOP.ADD_PRINT_BARCODE("6mm", "45mm","12mm", "10mm", "128A", "<%=orderCode.toUpperCase()%>");
		LODOP.ADD_PRINT_BARCODE("91mm", "9mm","12mm", "10mm", "128A", "<%=order.getCode()%>");
		LODOP.ADD_PRINT_BARCODE("137mm", "45mm","12mm", "10mm", "128A", "<%=orderCode.toUpperCase()%>");
		//LODOP.PREVIEWB();
		LODOP.PRINTB();
		return true;
	}else{
...
var b=initPrint();
if(b==true){
	window.location="<%=request.getContextPath()%>/admin/rec/oper/orderStock/scanOrderStock.jsp";
}
</script>
</body>
</html>