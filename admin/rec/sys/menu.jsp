...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/MenuController/getMenuTreeGrid.mmx',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
			...
			onAfterEdit : function(row, changes) {
				if (row.parentId == '' || row.parentId != row.id) {
					$.ajax({
						url : '${pageContext.request.contextPath}/MenuController/addOrEditMenuTreeGrid.mmx',
						data : row,
						cache : false,
						dataType : "json",
						success : function(r) {
	...
	function remove() {
		...
		if (node) {
			...
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】菜单？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/MenuController/delMenuTreeGrid.mmx',
						data : {
							id : node.id
						},
						cache : false,
...
<body class="easyui-layout" fit="true">
	...
	<div id="menu" class="easyui-menu" style="width:120px;display: none;">
		<div onclick="reload();" iconCls="icon-reload">刷新</div>
