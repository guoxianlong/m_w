...
<script type="text/javascript">
	...
	$(function(){
		var str = "areaId=3&storeType=0&status=0";
		$("#page_hidden").val(str);
		$("#storeType").combobox({
			valueField:'id',
			textField:'text',
		...
		$("#storageId").combobox({
			url:'<%=request.getContextPath()%>/CargoController/querySelectStorages.mmx?areaId='+3,
			valueField:'id',
			textField:'code',
		...
		$("#zccargoList").datagrid({
			...
			fitColumns:true,
			pageList:[5,10,15,20],
			queryParams:{areaId:3,storeType:0,status:0},
...
// 			                ]],
		...
		$("#queryZccargo").click(function(){
			...
			var temp = "wholeCode="+wholeCode+"&productCode="+productCode+"&stockCountStart="+stockCountStart
						...
						+"&stockAreaId="+stockAreaId+"&shelfCode="+shelfCode+"&floorNum="+floorNum+"&areaId="+3
