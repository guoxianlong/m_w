...
<body bgcolor="#ffcc00">
<form action=<%=request.getContextPath()+"/admin/mailingBatch.do?method=confirmMailingBalanceOrderReturn" %> method="post" onSubmit="return checksubmit();">
...
<form action=<%=request.getContextPath()+"/admin/mailingBatch.do?method=confirmMailingBalanceOrderReturnOperate" %> method="post" >
<table  width="99%" border="1" cellspacing="0" bordercolor="#00000">
  ...
  <%if(mbList!=null){
	    ...
	    %>
  <tr  bgcolor="#FFFFCC">
    <td><div align="center"><%=paging.getCountPerPage()*(paging.getCurrentPageIndex())+i+1%><input name="checkbox" type="checkbox"  name="checkbox" value="<%=mbBean.getId()%>"></div></td>
    <td><div align="center">
    	<%if(group.isFlag(430)){ %>
...
<br>
 <%if(group.isFlag(475)){ %>
<input type="submit" value="对当页选中退回订单确认并生成退货入库">
<%} %>
</form>
<%if (paging!=null){%>
